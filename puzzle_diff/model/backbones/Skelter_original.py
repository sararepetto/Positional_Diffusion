...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x, position):
        pe = torch.zeros(x.shape[1],self.max_len, self.ls)
        position = position.unsqueeze(1)
        new_position = torch.arange(0, self.max_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, self.ls, 2).float() * (-np.log(10000.0) / self.ls))
        pe[:,:, 0::2] = torch.sin((new_position * div_term)[position].squeeze())
        pe[:,:, 1::2] = torch.cos((new_position * div_term)[position].squeeze())
        pe = pe.transpose(0, 1)
        self.register_buffer('pe', pe)
        x = x[self.extra_pos_tokens:].to(position.device) + pe.to(position.device)
...
class CVAE(nn.Module):
    ...
    def encoder(self, skeletons, batch,position, aug='rotate',rot_heads = False):
        self.aug = aug 
        x = skeletons  #[S*T, J, C]
        batch = batch #[S]
        x = torch.stack(
        ...
        x = self.sequence_pos_encoder(x,position)  # [T, S, Ls]
        ################################################################################################################
        # transformer layers
