...
class Eff_GAT_In(nn.Module):
    ...
    def __init__(self, steps, input_channels=2, output_channels=2) -> None:
       ...
       # )
        ...
        self.combined_features_dim = 75 + 32 + 32 #visual features + pos_feats + temp_feats
        self.output_feature_dimension = 112
    ...
    def forward(self, xy_pos, time, patch_rgb, edge_index, batch):
        ...
        final_feats = self.forward_with_feats(
            xy_pos, time, patch_feats, edge_index, batch=batch
        )
        return final_feats
    ...
    def visual_features(self, patch_rgb):
        ...
        #patch_rgb = (patch_rgb - self.mean) / self.std
        #breakpoint()
        patch_feats = torch.flatten(patch_rgb, start_dim=1)
